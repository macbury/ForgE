package macbury.forge.graphics.fbo;

/**
 * Created by macbury on 20.07.15.
 */
public class Fbo {
  public final static String FRAMEBUFFER_MAIN_COLOR     = "Main Color";
  public final static String FRAMEBUFFER_SUN_FAR_DEPTH  = "Sun far Depth";
  public final static String FRAMEBUFFER_SUN_NEAR_DEPTH = "Sun near Depth";

  public static final String FRAMEBUFFER_REFLECTIONS = "Reflections";
  public static final String FRAMEBUFFER_REFRACTIONS = "Refractions";
  public static final String FRAMEBUFFER_FINAL       = "Final";
}
